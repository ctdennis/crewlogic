...
function buildPrompt(
  ...
  lines.push(`<time>, <City>: <one-line description of work>`);
  lines.push(`<time>, <City>: <one-line description of work>`);
  lines.push(`*NOTES:* <comma-separated callouts the crew needs to know — only include if non-obvious>`);
  ...
  lines.push(`  - The Notes label MUST be written as "*NOTES:*" — all caps, wrapped in single`);
  lines.push(`    asterisks. This renders as bold uppercase in WhatsApp and Slack where the`);
  lines.push(`    plan gets pasted. Do NOT use lowercase, do NOT omit the asterisks.`);
  lines.push('');

  lines.push(`RULES:`);
  lines.push(`  - One LINE per stop. Format: "<TIME>, <CITY>: <description>".`);
  lines.push(`    Examples:`);
  lines.push(`      "9AM, Plymouth: Single refrigerator, first floor."`);
  lines.push(`      "11:30AM Taunton: Basement cleanout"`);
  lines.push(`      "1PM Marion: 10x10 wood shed demolition ($1,613)"`);
  lines.push(`  - The stop line should describe WHAT the job is. No tools, no recycling streams,`);
  lines.push(`    no situational guidance — those go in Notes.`);
  lines.push(`  - "*NOTES:*" line: pack only the callouts that affect timing/billing/risk. Examples:`);
  lines.push(`      "*NOTES:* confirm Plymouth prior to arrival, possible long walk in Taunton"`);
  lines.push(`      "*NOTES:* Bridgewater VIP (realtor referrer), mattress recycling x2"`);
  lines.push(`      "*NOTES:* Marion shed - dense load expected"`);
  lines.push(`    Skip the *NOTES:* line entirely if nothing's worth flagging.`);
  if (toolsSpecialty.length > 0) {
