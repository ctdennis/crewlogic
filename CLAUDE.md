...
- **Run dev tooling in EXACTLY these forms** — relative path, from the repo root, **no `|`, no `>`/`2>&1`, no `;`/`&&`, no absolute paths.** Read the JSON/text output directly; if you must filter it, do that in a separate step.
  - **The three `dev-setup/*.sh` SQL wrappers self-target the CLI link (2026-07-25)** — each inspects the current `supabase link` and re-targets itself, then the read/write wrappers **always re-link back to dev** (the safe resting state). So you never hand-run `supabase link`, and a dev-intended query can't accidentally hit prod (or vice-versa). Raw `supabase db query --linked` is **retired for prod migrations** — it silently follows whatever's linked (usually dev); use `prod-write-sql.sh -f` instead.
  - Dev SQL (auto-links dev; allowlisted): `bash supabase/dev-setup/dev-sql.sh "<sql>"` (or `-f file.sql`)
  - **Read-only PROD SQL** (allowlisted; static read-only guards run FIRST, then auto-links prod, runs inside a `READ ONLY` transaction so Postgres blocks any write, then re-links dev): `bash supabase/dev-setup/prod-readonly-sql.sh "<SELECT ...>"`. Use this for prod SELECTs instead of `supabase db query` (which stays gated, since it can also write).
  - **PROD WRITE / migration** (auto-links prod → runs → always re-links dev, even on failure; **NOT allowlisted so every run prompts — that prompt is the human gate for a prod mutation**): `bash supabase/dev-setup/prod-write-sql.sh "<sql>"` or `bash supabase/dev-setup/prod-write-sql.sh -f supabase/migrations/NNNN_*.sql`. Replaces the old manual `link prod → db query --linked → link dev` dance.
  - Syntax-check index.html: `bash supabase/dev-setup/check-html.sh`
  - **Deploy a function — ALWAYS use the guardrail, never a raw `supabase functions deploy`:** `bash supabase/dev-setup/deploy-fn.sh <name> dev` (or `prod`, gated). The wrapper forces `--no-verify-jwt` for every must-be-public function (webhooks + crons + server-to-server) and verifies (by response BODY, not status code) that the gateway is actually open afterward. **Why this is mandatory:** a plain `supabase functions deploy` silently omits that flag → the Supabase gateway 401s Motive/Linxup signature-only posts and headerless crons → events are dropped fleet-wide and nobody notices (the 2026-07-30→08-03 incident: 4+ days of lost geofence events). If you add a new webhook/cron/server-to-server function, add it to the `PUBLIC` list in `deploy-fn.sh`. Backstop: `crewlogic-fn-health` (10-min cron) emails on any webhook gateway-401. Full RCA: `docs/plan-webhook-resilience.md`. Raw form (only if the script is unavailable): `supabase functions deploy --project-ref bagkimfwmpwjfhfhmsrb <name> --use-api --no-verify-jwt`
