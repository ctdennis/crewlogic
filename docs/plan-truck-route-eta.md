...
- **Mid-job dump-and-return (multi-truck big jobs).** A crew with two trucks can fill *mid-job*, both leave to a transfer station, **return to the same job** to finish, then split to their afternoon routes. Day-start can't foresee it (needs the capacity model above to predict the fill). **Live** mode *sees* it (GPS + facility geofences already exist) but needs a **dump-and-return detection rule**: a facility visit sandwiched between two visits to the SAME job = "still on that job," NOT "job done → next stop." Without it, live prematurely advances the prediction and mispredicts the afternoon split (which also starts later than scheduled because the dump run stretched the big job). Signals exist; the interpreting logic does not. Phase 2+.
