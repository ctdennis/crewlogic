...
- **Full cancel picklist** (BLOCKED ON OWNER — 2026-06-21 probe negative): the 974/975 option lists are **not exposed via the read API**. Verified read-only on dev: `/system/objects/` (objectID 10 = Job) returns only object metadata (`isHasFields:true`, no Fields array even with `isCompleteObject`); `/system/fields/` returns non-JSON; `/data/Jobs/` method 3 (Add scaffold) → errNo -501 "Incorrect parameters" (needs client/contact scaffolding); a live job read (`/data/Jobs/` method 1, job 855649) returns the 12 standard fields (status 984, etc.) but **not** 974/975/973 — those are cancel-scaffold-only. The only remaining API route is firing method 4 (the cancel-execute) to read back required-field errors, which is a real Vonigo write — NOT attempted. **Resolution:** Owner reads the optionIDs directly from the Vonigo cancel dropdowns (Customer Initiated / Pricing / Scheduling categories + each category's reasons). Only Scheduling→"Customer not ready" 10133/10129 captured so far. Until then, cancel ships with known codes + comment-973 fallback.
