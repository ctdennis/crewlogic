...
| Phase 3: RLS / SEC-1 | [CL-SPEC-004](Feature-StandaloneMode/CL-SPEC-004_Phase3_RLS_SEC1.md) | **In progress** (started 2026-06-03) | Dev only | v5.23.0 (dev) | 2026-06-03 | Done in dev: scope helpers (0006); client JWT in `supabaseFetch` (§6); dev bypass now mints a **real Supabase session** (`DEV_AUTH.md`); **`customers` RLS applied + verified** (0007 — dev-owner sees its 81, other franchise invisible). Next: roll remaining tables table-by-table; pre-auth carve-outs (§4). **Prod gating dep:** Google→Supabase Auth (link by email) so all prod users get `auth.uid()` |
