...
| Phase 3: RLS / SEC-1 | [CL-SPEC-004](Feature-StandaloneMode/CL-SPEC-004_Phase3_RLS_SEC1.md) | **Not started** (spec'd; pre-auth flows audited 2026-06-02) | n/a | — | 2026-06-02 | Today every RLS policy is `using(true)` + client is always `anon` → no DB-level isolation. Gating dep: all users need `auth.uid()` (Google owners still custom-session). Then scope-resolver helpers → per-table scoped policies → client sends user JWT. Carve-outs: invites-by-token, profiles bootstrap, feedback insert. Gates go-live |
