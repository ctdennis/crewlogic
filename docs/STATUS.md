...
| Phase 3: RLS / SEC-1 | [CL-SPEC-004](Feature-StandaloneMode/CL-SPEC-004_Phase3_RLS_SEC1.md) | **In progress** (started 2026-06-03) | Dev only | — | 2026-06-03 | Done: scope helpers (migration 0006); client JWT in `supabaseFetch` (§6); **franchise-scoping pattern proven** (rolled-back impersonation test); `customers` policy template (0007, not applied). **Blockers/decisions:** (1) dev sign-in bypass has no `auth.uid()` → need real dev sessions to test RLS in-browser; (2) Google→Supabase Auth so all users get `auth.uid()` (link vs re-provision). Then apply policies table-by-table → cross-tenant tests |
