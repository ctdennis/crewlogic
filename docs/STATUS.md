...
| Phase 3: RLS / SEC-1 | [CL-SPEC-004](Feature-StandaloneMode/CL-SPEC-004_Phase3_RLS_SEC1.md) | **In progress** (started 2026-06-03) | Dev only | v5.23.0 (dev) | 2026-06-03 | Done in dev: scope helpers (0006); client JWT in `supabaseFetch` (§6); dev bypass mints a **real Supabase session** (`DEV_AUTH.md`); **scoped RLS applied + verified on 16 tables** — customers (0007) + estimates, price_lists/blocks/items/zips, customer_price_lists, job_plans, crew_members, tools, campaigns, yard_signs+sign_* (0008). Verified in-browser (customers) + SQL (reads filter correctly). Plus pre-auth carve-outs done (0009: invites open-read/writes-scoped, feedback); `vonigo_credentials`/`_audit` already deny-all (edge-fn only). **Only profiles/franchises/tenants remain.** Google→Supabase Auth **verified mostly DONE in prod (2026-06-03):** Supabase Google OAuth is already the live path (9 google `auth.users`), 6/8 profiles linked. So no auth-method switch. **Auto-linking built (2026-06-03):** edge fn `crewlogic-link-identity` + wired into `resumeNativeSession` (dev, v5.23.1), verified end-to-end (re-links by email, idempotent). **ALL tables now scoped in dev** — profiles/franchises/tenants done (0010, email-fallback helpers for pre-link bootstrap; stranger-sees-0 verified). §8 storage policies **deferred** to a dedicated follow-up (storage runs through an anon client; scoping needs a client/grants rework — bounded gap, private bucket). Next = **prod cutover**: backfill gustavo/mark → deploy `crewlogic-link-identity` to prod → apply 0006–0010 to prod → push client (v5.23.2) |
