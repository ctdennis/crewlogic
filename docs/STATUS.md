...
| Town Price Lookup | [CL-SPEC-002](Feature-StandaloneMode/CL-SPEC-002_TownPriceLookup.md) | **Not started** (code-verified 2026-06-02: no `resolveTownToZip`/Zippopotam/town UI; `#priceLookupScreen` + `doZipLookup()` are still ZIP-only) | n/a | — | 2026-06-02 | Frontend-only: town+state UI, `resolveTownToZip()` (Zippopotam), wire into `doZipLookup()` (line ~6437), error handling, verify on Vonigo + native tenant |
