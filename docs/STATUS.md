...
| Phase 3: RLS / SEC-1 | [CL-SPEC-004](Feature-StandaloneMode/CL-SPEC-004_Phase3_RLS_SEC1.md) | **In progress** (started 2026-06-03) | Dev only | — | 2026-06-03 | Scope-resolver helpers built in dev (migration 0006: `current_franchise_id/tenant_id/user_role`). Next: Google→Supabase Auth so all users get `auth.uid()` (dev: 2/3 profiles missing it) — **decision pending** (link vs re-provision). Then client JWT (no-op) → per-table scoped policies → cross-tenant tests. Carve-outs: invites-by-token, profiles bootstrap, feedback insert |
