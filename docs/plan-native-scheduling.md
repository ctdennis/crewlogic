...
> **DECISIONS 2026-07-18.** Owner-confirmed: **Q-B** (manual booking/dispatch = Starter baseline; **online booking = the Pro differentiator**) and the **P0-first** sequencing of Q-D. Master-applied defaults per the auto-apply triage — veto any: **Q-A** day-level v1, **Q-D** P0 in parallel, **Q-E** online booking as its own sub-plan (design `jobs.origin` hooks now), **Q-H** relational reference tables (not blobs), **Q-I** light calendar visibility in v1.
>
> **PAYMENT DIRECTION — DECIDED 2026-07-18 (supersedes the Q-G lean below).** Owner chose **on-site payment capture by crews** as the first money surface, with the **invoicing foundation built alongside** so invoicing can layer on later without rework. **Payment attaches to the JOB — NOT to the estimate.** Master proposed an estimate-scoped shortcut (payment against `estimates`, which native already has, so payment could ship before the jobs table); **Owner rejected it: "not looking for a short cut that will never be used."** The jobs table is therefore a hard prerequisite for any payment work — build P1 first, then payment capture against `jobs`. Do not re-propose an estimate-scoped payment path.

- **Q-A. Scheduling granularity for v1** — day-level (assign a job to a day + crew, board reorders within the day, matches the current 2-day board), or time-slot (start/end times, calendar grid)? **APPLIED: day-level v1** — matches the existing board; online self-booking (P3) forces slots + an availability model, so that is where times land.
- **Q-B. Starter/Pro line for scheduling — DECIDED (2026-07-18):** **manual booking + dispatch = Starter** (native baseline — native tenants book their own appointments from day one); **online customer self-booking = Pro**, together with its availability/capacity engine.
