...
- **P4 — Dispatch follow-ups rail + day-dots. ✅ dev (v5.122.0).** A "🔔 Follow-ups due" rail on the Dispatch screen (Overdue/Today/This-week, contact chips + Done/Snooze) + a "🔔 N" due-count chip on each board's day picker. New lean `reminders` server action feeds both. Rail is self-contained (doesn't touch the route grid); no auto-poll. Gated dev/super-admin/tester.
